 ...
 *                                   
   ...
   10% Supply for marketing wallet
   5% Supply locked for 30 days for special announcement
...
contract testmoon2 is Context, IERC20, Ownable {
	...
	address public charityWallet = address(0x37C27C32769EAd601fB744294d981d9D67cebd8B); // will be adjusted later
	...
	uint256 public _maxWalletToken = 5000000 * 10**9; // 0.5% of total supply after burn
	...
	function _transfer(
		...
		if (to != owner() && to != address(this) && to != uniswapV2Pair && to != address(1)) {
			...
			if (block.number - deployedAtBlock < 500) { // if not 48 hours has passed, set the buy limit
